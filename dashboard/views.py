...
def dashboard_home(request):
    ...
    successful_payments = Payment.objects.filter(payment_status="success").order_by(
        "-payment_date"
    )
    ...
    # ------------------------------------------------------------------
    monthly_data = defaultdict(lambda: {"flights": 0, "hotels": 0, "packages": 0})
    ...
    for booking in flight_bookings_qs:
        if booking.flight:
            route = (
                f"{booking.flight.source.code} → " f"{booking.flight.destination.code}"
            )
    ...
    flight_revenue = (
        Payment.objects.filter(
            payment_status="success", booking__isnull=False
        ).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    hotel_revenue = (
        Payment.objects.filter(
            payment_status="success", hotel_booking__isnull=False
        ).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    package_revenue = (
        Payment.objects.filter(
            payment_status="success", package_booking__isnull=False
        ).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    ...
    revenue_today = (
        Payment.objects.filter(
            payment_status="success", payment_date__date=today
        ).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    ...
    revenue_this_month = (
        Payment.objects.filter(
            payment_status="success", payment_date__date__gte=month_start
        ).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    ...
    # ------------------------------------------------------------------
    avg_booking_value = (
        float(total_revenue) / total_bookings if total_bookings > 0 else 0
    )

    cancellation_pct = (
        round((total_cancelled / total_bookings) * 100, 1) if total_bookings > 0 else 0
    )
    ...
    most_booked_package = (
        top_package_destinations[0][0] if top_package_destinations else "N/A"
    )
    ...
    if customer_spending:
        ...
        top_spender = max(customer_spending.values(), key=lambda x: x["spent"])
    ...
    recent_flight_bookings = list(
        flight_bookings_qs.order_by("-created_at").select_related(
            "user", "flight", "flight__source", "flight__destination"
        )[:10]
    )
    recent_hotel_bookings = list(
        hotel_bookings_qs.order_by("-created_at").select_related("user", "hotel")[:10]
    )
    recent_pkg_bookings = list(
        package_bookings_qs.order_by("-created_at").select_related("user", "package")[
            :10
        ]
    )
    ...
    # ------------------------------------------------------------------
    recent_payments_extended = Payment.objects.select_related(
        "booking__user", "hotel_booking__user", "package_booking__user"
    ...
    top_customers = sorted(
        customer_stats.values(), key=lambda x: x["total_spent"], reverse=True
    )[:5]
    ...
    highest_rev_day_label = (
        highest_revenue_day["day"].strftime("%d %b %Y")
        if highest_revenue_day
        else "N/A"
    )
    highest_rev_day_amount = (
        float(highest_revenue_day["rev"]) if highest_revenue_day else 0
    )
    lowest_rev_day_label = (
        lowest_revenue_day["day"].strftime("%d %b %Y") if lowest_revenue_day else "N/A"
    )
    lowest_rev_day_amount = (
        float(lowest_revenue_day["rev"]) if lowest_revenue_day else 0
    )
...
def booking_trend(request):
    ...
    hotel_trend = (
        HotelBooking.objects.annotate(day=TruncDate("created_at"))
        .values("day")
        .annotate(total=Count("id"))
        .order_by("day")
    )
    ...
    package_trend = (
        PackageBooking.objects.annotate(day=TruncDate("created_at"))
        .values("day")
        .annotate(total=Count("id"))
        .order_by("day")
    )
    ...
    popular_routes = (
        Booking.objects.filter(flight__isnull=False)
        .values(
    ...
    for booking in Booking.objects.all():
        ...
        weekday_counter[booking.created_at.strftime("%A")] += 1

        hour_counter[booking.created_at.hour] += 1
    ...
    weekday_data = [weekday_counter.get(day, 0) for day in weekday_order]

    hour_labels = list(range(24))

    hour_data = [hour_counter.get(hour, 0) for hour in hour_labels]

    peak_day = max(weekday_counter, key=weekday_counter.get, default="N/A")

    peak_hour = max(hour_counter, key=hour_counter.get, default=0)

    # ----------------------------------------------------
    # Cancellation Analytics
    # ----------------------------------------------------

    flight_cancelled = Booking.objects.filter(booking_status="cancelled").count()

    hotel_cancelled = HotelBooking.objects.filter(booking_status="cancelled").count()

    package_cancelled = PackageBooking.objects.filter(
        booking_status="cancelled"
    ).count()

    total_cancelled = flight_cancelled + hotel_cancelled + package_cancelled
    ...
    cancellation_rate = (
        ...
        if total_bookings_all
        else 0
    )

    refund_total = Booking.objects.aggregate(total=Sum("refund_amount"))["total"] or 0
    ...
    average_booking_value = (
        Payment.objects.filter(payment_status="success").aggregate(avg=Avg("amount"))[
            "avg"
        ]
        or 0
    )
    ...
    repeat_percentage = (
        round((repeat_customers / total_customers) * 100, 1) if total_customers else 0
    )
    ...
    for customer in top_customers:
        ...
        customer_segments.append(
            {
                "name": (
                    f'{customer["user__first_name"]} ' f'{customer["user__last_name"]}'
                ).strip(),
    ...
    status_labels = [item["booking_status"].title() for item in booking_status]

    status_data = [item["total"] for item in booking_status]
    ...
    payment_labels = [item["payment_method"].upper() for item in payment_methods]

    payment_data = [item["total"] for item in payment_methods]
    ...
    revenue_history = [float(item["revenue"]) for item in daily_revenue]
    ...
    if revenue_history:
        ...
        forecast_labels = [f"Day {i}" for i in range(1, 31)]
    ...
    if len(revenue_history) >= 5:
        ...
        rmse = round(
            (sum((a - p) ** 2 for a, p in zip(actual, predicted)) / len(actual)) ** 0.5,
            2,
        )
    ...
    average_refund = (
        Booking.objects.filter(booking_status="cancelled").aggregate(
    ...
    cancelled_bookings = (
        Booking.objects.filter(booking_status="cancelled")
    ...
    monthly_labels = [item["month"].strftime("%b %Y") for item in monthly_bookings]

    monthly_data = [item["total"] for item in monthly_bookings]
    ...
    context = {
        "flight_labels": [x["day"].strftime("%d %b") for x in flight_trend],
        "flight_data": [x["total"] for x in flight_trend],
        "hotel_labels": [x["day"].strftime("%d %b") for x in hotel_trend],
        "hotel_data": [x["total"] for x in hotel_trend],
        "package_labels": [x["day"].strftime("%d %b") for x in package_trend],
        "package_data": [x["total"] for x in package_trend],
    ...
    return render(request, "dashboard/booking_trend.html", context)
