...
def flight_search(request):
    ...
    if form.is_valid():
        ...
        if source == destination:
            ...
            form.add_error(None, "Source and destination cannot be same")
            ...
            for flight in direct_flights:
                ...
                ) = get_confidence(flight.recommendation_score)
                ...
                if preferences:
                    if (
                        ...
                        and flight.airline == preferences["preferred_airline"]
                    ):
            ...
            first_legs = (
                Flight.objects.filter(source=source, available_seats__gte=passengers)
            ...
            for first_leg in first_legs:
                ...
                earliest_departure = first_leg.arrival_time + timedelta(
                    minutes=layover_min
                )

                latest_departure = first_leg.arrival_time + timedelta(
                    minutes=layover_max
                )
                ...
                if second_leg:
                    ...
                    total_price = (first_leg_price + second_leg_price) * passengers

                    total_duration = int(
                        (
                            second_leg.arrival_time - first_leg.departure_time
                        ).total_seconds()
                        / 60
                    )
                    ...
                    if preferences:
                        ...
                        if (
                            preferences["preferred_airline"]
                            and first_leg.airline == preferences["preferred_airline"]
                        ):
                    ...
                    booking_count = trending.get(first_leg.id, 0) + trending.get(
                        second_leg.id, 0
                    )
...
def flight_detail(request, flight_id):
    ...
    if is_via:
        ...
        second_leg_id = request.GET.get("second_leg_id")
        ...
        if selected_date:
            ...
            if second_leg:
                ...
                second_leg.departure_time = second_leg.departure_time.replace(
                    year=selected_date.year,
                    month=selected_date.month,
                    day=selected_date.day,
                )

                second_leg.arrival_time = second_leg.arrival_time.replace(
                    year=selected_date.year,
                    month=selected_date.month,
                    day=selected_date.day,
                )
    ...
    # ==================================
    average_rating = flight.reviews.aggregate(Avg("rating"))["rating__avg"]
    ...
    try:
        ...
        predicted_price = fare_predictor.predict(
            ...
            destination=(
                second_leg.destination.city if second_leg else flight.destination.city
            ),
    ...
    context = {
        ...
        "second_leg_flight_number_display": (
            second_leg.get_display_flight_number() if second_leg else None
        ),
