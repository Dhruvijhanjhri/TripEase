...
def split_interests(interests_text):
    ...
    return [item.strip().lower() for item in interests_text.split(",") if item.strip()]


def get_matching_hotels(destination, budget, days):
    destination = normalize_text(destination)

    hotels = Hotel.objects.filter(city__icontains=destination).order_by(
    ...
    estimated_hotel_cost = sum(selected_room_prices, Decimal("0.00")) * max(days - 1, 1)
...
def get_matching_packages(destination, budget, days, interests):
    ...
    matched_within_budget.sort(key=lambda x: (x[0], x[1].rating), reverse=True)
    matched_fallback.sort(key=lambda x: (x[0], x[1].rating), reverse=True)

    selected = [pkg for score, pkg in matched_within_budget[:3] if score > 0]

    if not selected:
        selected = [pkg for score, pkg in matched_fallback[:3] if score > 0]
    ...
    estimated_package_cost = sum(selected_package_prices, Decimal("0.00"))
...
def build_day_wise_plan(destination, days, interests):
    interest_list = split_interests(interests)
    interest_text = ", ".join(interest_list) if interest_list else "local sightseeing"
    ...
    for day in range(1, days + 1):
        if day == 1:
            activity = (
                f"Arrival in {destination}, hotel check-in " f"and nearby exploration."
            )
        elif day == days:
            activity = (
                f"Relaxed morning, local shopping and " f"departure from {destination}."
            )
        else:
            activity = (
                f"Day {day} activities based on your interests: " f"{interest_text}."
            )
...
def generate_trip_plan(destination, budget, days, interests, origin_city="Bengaluru"):
    ...
    itinerary_text = generate_ai_itinerary(destination, days, budget, interests)

    estimated_cost = estimate_total_cost(hotel_cost, package_cost, budget, days)
