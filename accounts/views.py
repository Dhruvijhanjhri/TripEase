...
try:
    ...
    logger.warning(
        "Supabase client not available. Authentication will use Django's default system."
    )
                ...
                if SUPABASE_AVAILABLE:
                    try:
                        ...
                        if auth_response.user:
                            ...
                            logger.info(
                                f"User created in Supabase: {email} with ID: {supabase_user_id}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Supabase signup failed, falling back to Django: {str(e)}"
                        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            email = form.cleaned_data[
                "username"
            ]  # LoginForm uses 'username' field for email
            ...
            if SUPABASE_AVAILABLE:
                try:
                        ...
                        try:
                            ...
                            if user_metadata:
                                django_user.first_name = (
                                    ...
                                    or django_user.first_name
                                )
                                django_user.last_name = (
                                    user_metadata.get(
                                        "last_name", django_user.last_name
                                    )
                                    or django_user.last_name
                                )
                                django_user.phone = (
                                    user_metadata.get("phone", django_user.phone)
                                    or django_user.phone
                                )
                    ...
                    logger.warning(
                        f"Supabase login failed, trying Django auth: {str(e)}"
                    )
        ...
        if form.is_valid():
            ...
            if SUPABASE_AVAILABLE:
                try:
                    ...
                    logger.warning(
                        f"Supabase password reset failed for {email}: {str(e)}"
                    )
